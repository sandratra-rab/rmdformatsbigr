...
readthebigrdoc <- function(fig_width = 8,
                       fig_height = 5,
                       fig_caption = TRUE,
                       highlight = "kate",
                       lightbox = FALSE,
                       ...
                       pandoc_args = NULL,
                       md_extensions = NULL,
                       mathjax = "rmdformatsbigr",
                       ...) {

    html_template(
        template_name = "readthebigrdoc",
        template_path = "templates/template.html",
        template_dependencies = list(
            html_dependency_readthebigrdoc(embed_fonts)
        ),
...
# readthebigrdoc js and css
html_dependency_readthebigrdoc <- function(embed_fonts = TRUE) {
  stylesheets <- "readthebigrdoc.css"
  if (embed_fonts) {
    stylesheets <- c(stylesheets, "readthebigrdoc_fonts_embed.css")
  } else {
    stylesheets <- c(stylesheets, "readthebigrdoc_fonts_download.css")
  }
  htmltools::htmlDependency(name = "readthebigrdoc",
                 version = "0.1",
                 src = system.file("templates/readthebigrdoc", package = "rmdformatsbigr"),
